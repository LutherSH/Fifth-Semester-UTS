...
public class WeaponSwitching : MonoBehaviour
{
    ...
    private void UpdateAmmoUIForWeapon(int weaponIndex)
    {
        if (ammoUI == null)
        {
            ...
            if (ammoUI == null)
            {
                Debug.LogWarning("AmmoUI not found!");
                return;
            }
        }

        // Get gun component from selected weapon
        WeaponBehaviour selectedWeapon = weapons[weaponIndex].GetComponent<WeaponBehaviour>();
        if (selectedWeapon != null)
        {
            ammoUI.SubscribeToWeapon(selectedWeapon);
        }
        else
        {
            Debug.LogWarning("WeaponBehaviour component not found on weapon: " + weapons[weaponIndex].name);
        }
    }
}